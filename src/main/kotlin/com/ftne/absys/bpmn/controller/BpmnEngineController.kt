...
class BpmnEngineController(
    ...
    @PutMapping("/process/{id}/complete/manual")
    fun completeHumanTask(@PathVariable id: UUID) {
        processService.completeHumanTask(id)
    }

    @PutMapping("/process/{id}/correlation/{msgName}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    fun correlateMessage(@PathVariable id: UUID, @PathVariable msgName: String) {
        processService.correlateMessage(id, msgName)
    }
